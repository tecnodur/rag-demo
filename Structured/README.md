...
- Loads `metadata_manifest.json`.
- Parses the metadata block at the top of each markdown file.
...
- Retrieved chunks for structured retrieval.
- Parent title, version, status, region, and score for each hit.
- Gemini's final answer for both modes.
- Naive mode intentionally withholds metadata from generation, so it can answer from semantically similar but outdated content.

You can also override the question or structured filters:
...
python3 scripts/run_demo.py --naive-limit 8 --structured-limit 4
```
...
- Searches the same `knowledge_chunks` collection.
- Applies metadata constraints before generation.
- Uses `region=EU`, `status=Active`, and `doc_type=policy`.
- Prefers the newest matching version if multiple applicable policy versions are retrieved.
- Should surface `POL-EU-PWD-003` as the authoritative answer source.
...
- If retrieval does use metadata, the system can constrain to the active EU policy before generation.
...
- Embeds the same user question.
- Runs Atlas `$vectorSearch` with metadata filters decided by the retrieval code, not by the model.
- Reranks matching hits to prefer active and newer versions.

This is the presentation point: same data, same model family, different retrieval discipline.

## Troubleshooting

If ingestion fails with a metadata mismatch:

- Check the metadata block at the top of the markdown file.
- Check the corresponding entry in `data/source_docs/metadata_manifest.json`.
- The loader fails intentionally when they disagree.
...
- Inspect the printed retrieved chunks in each mode.
- Confirm structured mode is using `--region EU --status Active --doc-type policy`.
- Re-run ingestion if you changed the source documents or metadata.
